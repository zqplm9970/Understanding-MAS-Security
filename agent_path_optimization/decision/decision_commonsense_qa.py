...
def build_safety_vectors(all_answers: Dict[str, str], ground_truth: str) -> Dict[str, Dict]:
    ...
    for aid in agent_ids:
        ans = str(all_answers.get(aid, "")).strip().upper()
        valid = ans in ['A', 'B', 'C', 'D', 'E']
        is_correct = valid and (ans == ground_truth)
        u_t = 1.0 if valid else 0.0

        others = [str(all_answers[o]).strip().upper() for o in agent_ids if o != aid]
        if others:
            same_count = sum(1 for o_ans in others if o_ans == ans and ans in ['A', 'B', 'C', 'D', 'E'])
            c_t = same_count / len(others)
        else:
            c_t = 0.0
...
def run_decision_center_process(full_question: str, question_idx: int, ground_truth: str) -> Dict:
    ...
    for aid, vec in safety_vectors.items():
        print(f"  {aid}: ans={vec['answer']}, correct={vec['is_correct']}, "
    ...
    answers_summary = "The agents involved in the final decision-making process and their answers are as follows:\n"
    ...
    try:
        chat_result = user_proxy.initiate_chat(
            ...
            message=(
                f"{answers_summary}\n"
                f"Based on the answers from these agents, please determine the system's final answer."
                f"Please directly output the option letter (A, B, C, D, or E)."
            ),
            clear_history=False,
            silent=True
        )
    ...
    question_result["is_correct"] = (step2_result["final_answer_cleaned"] == ground_truth)
